...
import { ConflictErrorDialog } from '@/components/dialogs/ConflictErrorDialog';
import { 
  Package, 
  Info,
  Calculator,
...
import { ErrorDetails, ConflictErrorData } from '@/types/error';
...
export function ItemForm({
  ...
  const itemValidation = useItemValidation({
    onValidationError: (errorDetails, conflictData) => {
      setErrorDialog({
        open: true,
        errorDetails,
        conflictData
  ...
  // Enhanced submission mutation with error handling
  const submitMutation = useMutation({
    ...
    onError: (error: AxiosError) => {
      ...
      const conflictData = ErrorParser.parseConflictError(error);
  ...
  // Debounced item name validation
  ...
  const handleItemNameChange = useCallback((value: string) => {
    ...
    if (value && value.trim() && mode === 'create') {
      const timeout = setTimeout(() => {
        itemValidation.validateItemName(value.trim());
      }, 800); // 800ms delay
      setNameValidationTimeout(timeout);
    } else {
      itemValidation.clearValidation();
    }
  }, [itemValidation, mode, nameValidationTimeout]);

  const handleFormSubmit = async (data: ItemFormData) => {
    // Store form data for potential retry
    setFormData(data);

    // Validate full item data before submission
    if (mode === 'create') {
      itemValidation.validateItem({
        item_name: data.item_name,
        sku: '', // Generated by backend
        ...
        is_saleable: data.is_salable ?? false,
        item_status: 'ACTIVE'
      });
      
      // Don't submit if validation is in progress or failed
      if (itemValidation.isValidating || !itemValidation.isItemNameValid) {
        return;
      }
    }
    ...
    switch (action) {
      case 'suggest_alternatives':
        if (data?.suggestedName) {
          ...
          // Clear validation state and retry
          itemValidation.clearValidation();
        }
        break;
      ...
      case 'view_existing':
        if (data?.resourceId) {
          // Navigate to existing item (implement based on your routing)
          addNotification({
            type: 'info',
            title: 'Existing Item',
            message: 'Opening existing item details...'
          });
        }
        setErrorDialog({ open: false });
        break;
  ...
  }, [formData, form, itemValidation, addNotification, submitMutation]);
    ...
    <form id={formId} onSubmit={handleSubmit(handleFormSubmit)} className={cn('space-y-4', className)}>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
            <div className="space-y-2">
                ...
                <div className="flex-1">
                  <Input
                    ...
                    className={cn(
                      ...
                      itemValidation.isValidating ? 'border-yellow-400' : '',
                      !itemValidation.isItemNameValid && itemValidation.hasValidated ? 'border-red-500' : ''
                    )}
                  />
                </div>
              ...
              {itemValidation.isValidating && (
                <p className="text-sm text-yellow-600">Checking for duplicates...</p>
              )}
              {!itemValidation.isItemNameValid && itemValidation.hasValidated && (
                <div className="text-sm text-red-600">
                  {itemValidation.validationErrors.map((error, index) => (
                    <p key={index}>{error}</p>
                  ))}
                </div>
              )}
              {itemValidation.validationSuggestions.length > 0 && (
                <div className="text-sm text-blue-600">
                  {itemValidation.validationSuggestions.map((suggestion, index) => (
      ...
      {!hideButtons && (
        <div className="flex items-center justify-end space-x-4 pt-4">
          ...
          <Button
            type="submit"
            disabled={isSubmitting || !isFormValid || itemValidation.isValidating || (!itemValidation.isItemNameValid && itemValidation.hasValidated) || submitMutation.isPending}
            className={cn(
              "min-w-[120px] transition-colors duration-200",
              isFormValid && !isSubmitting && !itemValidation.isValidating && (itemValidation.isItemNameValid || !itemValidation.hasValidated) && !submitMutation.isPending
      ...
      <ConflictErrorDialog
        ...
        conflictData={errorDialog.conflictData}
