# app/schemas/supplier.py  (100% Pydantic v2)
...
from app.models.supplier import SupplierType, SupplierTier, SupplierStatus, PaymentTerms
...
def _validate_website(v: Optional[str]) -> Optional[str]:
...
class SupplierCreate(BaseModel):
    ...
    address_line1: Optional[str] = Field(
        None, max_length=255, description="Primary address line"
    )
    ...
    credit_limit: Decimal = Field(
        Decimal("0.00"), ge=0, description="Credit limit amount"
    )
    supplier_tier: SupplierTier = Field(
        SupplierTier.STANDARD, description="Supplier tier"
    )
    status: SupplierStatus = Field(SupplierStatus.ACTIVE, description="Supplier status")
    notes: Optional[str] = Field(None, description="Additional notes")
    website: Optional[str] = Field(
        None, max_length=255, description="Supplier website"
    )
    account_manager: Optional[str] = Field(
        None, max_length=255, description="Account manager name"
    )
    preferred_payment_method: Optional[str] = Field(
        None, max_length=50, description="Preferred payment method"
    )
    insurance_expiry: Optional[datetime] = Field(
        None, description="Insurance expiry date"
    )
    ...
    contract_end_date: Optional[datetime] = Field(
        None, description="Contract end date"
    )
...
class SupplierUpdate(BaseModel):
    company_name: Optional[str] = Field(
        None, max_length=255, description="Supplier company name"
    )
...
class SupplierContactUpdate(BaseModel):
    ...
    email: Optional[str] = Field(
        None, max_length=255, description="Supplier email address"
    )
    phone: Optional[str] = Field(None, max_length=50, description="Supplier phone number")
    mobile: Optional[str] = Field(None, max_length=50, description="Supplier mobile number")
    website: Optional[str] = Field(
...
class SupplierContractUpdate(BaseModel):
    ...
    credit_limit: Optional[Decimal] = Field(
        None, ge=0, description="Credit limit amount"
    )
    supplier_tier: Optional[SupplierTier] = Field(None, description="Supplier tier")
    insurance_expiry: Optional[datetime] = Field(
        None, description="Insurance expiry date"
...
class SupplierSearchRequest(BaseModel):
    search_term: str = Field(
        ..., min_length=2, max_length=100, description="Search term"
    )
    ...
    min_quality_rating: Optional[float] = Field(
        None, ge=0, le=5, description="Minimum quality rating"
    )
    max_quality_rating: Optional[float] = Field(
        None, ge=0, le=5, description="Maximum quality rating"
    )
    min_delivery_rating: Optional[float] = Field(
        None, ge=0, le=5, description="Minimum delivery rating"
    )
    max_delivery_rating: Optional[float] = Field(
        None, ge=0, le=5, description="Maximum delivery rating"
    )
    contract_expiring_days: Optional[int] = Field(
        None, ge=0, le=365, description="Contract expiring within days"
    )
...
class SupplierPerformanceUpdate(BaseModel):
    quality_rating: Optional[float] = Field(
        None, ge=0, le=5, description="Quality rating (0-5)"
    )
    delivery_rating: Optional[float] = Field(
        None, ge=0, le=5, description="Delivery rating (0-5)"
    )
    average_delivery_days: Optional[int] = Field(
        None, ge=0, description="Average delivery time in days"
    )
    total_orders: Optional[int] = Field(None, ge=0, description="Total number of orders")
    total_spend: Optional[Decimal] = Field(
        None, ge=0, description="Total amount spent"
    )
