...
from sqlalchemy import Column, String, Text, Boolean, DateTime, Numeric, Integer, Index, CheckConstraint
...
class Supplier(RentalManagerBaseModel):
    """
    ...
    supplier_type = Column(String(20), nullable=False, comment="Type of supplier")
    ...
    __table_args__ = (
        ...
        CheckConstraint("supplier_type IN ('MANUFACTURER', 'DISTRIBUTOR', 'WHOLESALER', 'RETAILER', 'INVENTORY', 'SERVICE', 'DIRECT')", name='check_supplier_type'),
        CheckConstraint("supplier_tier IN ('PREMIUM', 'STANDARD', 'BASIC', 'TRIAL')", name='check_supplier_tier'),
        CheckConstraint("status IN ('ACTIVE', 'INACTIVE', 'PENDING', 'APPROVED', 'SUSPENDED', 'BLACKLISTED')", name='check_supplier_status'),
        CheckConstraint("payment_terms IN ('IMMEDIATE', 'NET15', 'NET30', 'NET45', 'NET60', 'NET90', 'COD')", name='check_payment_terms'),
        ...
        CheckConstraint("average_delivery_days >= 0", name='check_average_delivery_days'),
        CheckConstraint("total_orders >= 0", name='check_total_orders'),
        CheckConstraint("total_spend >= 0", name='check_total_spend'),
