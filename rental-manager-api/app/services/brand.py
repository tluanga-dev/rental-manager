...
class BrandService:
    ...
    async def list_brands(
        ...
        brands_list, total = await self.repository.get_paginated(
            page=page,
            page_size=page_size,
            filters=filter_dict,
            sort_by=sort_by,
        ...
        for brand in brands_list:
            summary = await self._to_summary(brand)
    ...
    async def search_brands(
        ...
        return [await self._to_summary(brand) for brand in brands]
    ...
    async def export_brands(
        ...
        for brand in brands:
            ...
            # Add item count (placeholder until items relationship available)
            export_item.item_count = 0
    ...
    async def _to_summary(self, brand: Brand) -> BrandSummary:
        """Convert brand model to summary schema.
        ...
        created_at = getattr(brand, 'created_at', None)
        ...
        return BrandSummary(**brand_dict)