...
class BrandRepository:
    ...
    async def get_paginated(
        ...
        """Get paginated brands with total count."""
        query = select(Brand)
    ...
    async def get_brands_with_items(self) -> List[Brand]:
        ...
        # This will be implemented when items relationship is available
        # For now, return empty list
        return []
    
    async def get_brands_without_items(self) -> List[Brand]:
        """Get brands that have no associated items."""
        # For now, return all brands since items relationship is not implemented
        query = select(Brand).where(Brand.is_active == True).order_by(Brand.name)
        result = await self.session.execute(query)
        return result.scalars().all()
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        # Count brands with items (placeholder until items relationship is available)
        brands_with_items = 0
