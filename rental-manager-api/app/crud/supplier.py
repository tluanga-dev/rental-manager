...
from app.models.supplier import Supplier, SupplierType, SupplierTier, SupplierStatus, PaymentTerms
...
class SupplierRepository:
    ...
    async def create(self, supplier_data: dict) -> Supplier:
    ...
    async def update(self, supplier_id: UUID, supplier_data: dict) -> Optional[Supplier]:
        """Update a supplier."""
        query = select(Supplier).where(Supplier.id == supplier_id)
        result = await self.session.execute(query)
    ...
    async def search(
        ...
        # Order by relevance (exact matches first) - simplified
    ...
    async def get_suppliers_with_expiring_contracts(
        ...
        now = datetime.now(timezone.utc).replace(tzinfo=None)  # Remove timezone info
        ...
        query = select(Supplier).where(
            and_(
                ...
                Supplier.contract_end_date >= now
            )
        )
