...
from app.shared.utils.category_code_generator import CategoryCodeGenerator
...
class CategoryService:
    ...
    async def update_category(
        ...
        # Update description
        if hasattr(category_data, 'description') and category_data.description is not None:
        ...
        if path_update_needed:
            ...
            if parent_changed:
                if path_parent_id:
                    new_parent = await self.repository.get_by_id(path_parent_id)
                    update_data["category_level"] = new_parent.category_level + 1
                else:
                    update_data["category_level"] = 1
        ...
        if path_update_needed:
            if parent_changed:
                await self._update_descendant_paths_after_parent_change(updated_category)
            else:
    ...
    async def _validate_parent_change(
        ...
        # Check if already under this parent (though this is checked in update_category)
        if new_parent_id == category.parent_category_id:
            raise BusinessRuleError("Category is already under this parent")
    ...
    async def _update_descendant_paths_after_parent_change(self, category: Category):
        """Update descendant paths after category parent change."""
        descendants = await self.repository.get_descendants(category.id)
        
        for descendant in descendants:
            # Calculate new path based on the updated parent category
            # The descendant's relative path from the moved category stays the same
            moved_category_segments = category.get_path_segments()
            descendant_segments = descendant.get_path_segments()
            
            # Get the relative path from the moved category to the descendant
            category_depth = len(moved_category_segments)
            descendant_relative_segments = descendant_segments[category_depth:]
            
            # Build new path: new category path + relative segments
            new_path_segments = moved_category_segments + descendant_relative_segments
            new_path = "/".join(new_path_segments)
            
            # Calculate new level
            new_level = len(new_path_segments)
    ...
    async def _to_summary(self, category: Category) -> CategorySummary:
        ...
        category_id = category.id
        category_name = category.name
        category_code = category.category_code  # Fix: Add missing category_code field
        category_path = category.category_path
        category_level = category.category_level
        parent_category_id = category.parent_category_id
        display_order = category.display_order
        ...
        return CategorySummary(
            id=category_id,
            name=category_name,
            category_code=category_code,  # Fix: Include category_code in the response
            category_path=category_path,
            category_level=category_level,
            parent_category_id=parent_category_id,
            display_order=display_order,
