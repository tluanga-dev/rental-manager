...
from app.modules.transactions.rentals.schemas import NewRentalRequest
...
class RentalsRepository:
    ...
    async def _insert_header(
        ...
        for it in dto.items:
            ...
            subtotal += (qty * unit_rate - discount).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    ...
    async def _bulk_insert_lines_and_movements(
        ...
        for idx, it in enumerate(dto.items):
            ...
            discount = Decimal(str(it.discount_value or 0))
            line_total = (qty * unit_price - discount).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            ...
            moves.append(
                StockMovement(
                    ...
                    transaction_header_id=header.id,
                    transaction_line_id=lines[-1].id,
                    movement_type="STOCK_MOVEMENT_RENTAL_OUT",
                    quantity_change=-qty,
                    quantity_before=sl.quantity_on_hand,
                    quantity_after=sl.quantity_on_hand - qty,
                    reason=f"Rental {header.transaction_number}",
    ...
    async def get_rentable_items_with_stock(
        ...
        stmt = (
            ...
            .where(Item.is_active.is_(True), Item.is_rentable.is_(True))
            .group_by(
                Item.id,
                Item.item_name,
                Category.name,
