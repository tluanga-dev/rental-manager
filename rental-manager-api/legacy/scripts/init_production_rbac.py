#!/usr/bin/env python3
"""
Production RBAC Initialization Script
Ensures RBAC data is properly seeded in production
This script is designed to be run during Railway deployment
"""
...
async def init_rbac():
    """Initialize RBAC data in production database"""
    try:
        ...
        print("🚀 Production RBAC Initialization")
        print("=" * 60)
        ...
        async for session in get_db():
            try:
                ...
                if role_created_count > 0:
                    await session.commit()
                    print(f"✅ Created {role_created_count} new roles")
                else:
                ...
                if admin_user:
                    ...
                    if super_admin_role and super_admin_role not in admin_user.roles:
                        ...
                        print(f"ℹ️  Admin user already has SUPER_ADMIN role")
                else:
                    print(f"⚠️  Admin user not found - will be created by another script")
